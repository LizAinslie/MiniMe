exports.run = (client, message, args) => {
    ...
    if (choices.length <= 1) return message.channel.createMessage(':question:	׀ You must have more than one choice.')
		message.channel.createMessage(':thinking: │ `' + choices[Math.floor(Math.random() * choices.length)] + '`')
	} else {
		message.channel.createMessage(':question: │ Missing `<choice 1>; <choice 2>; <choice 3>` option.')
	}
}
